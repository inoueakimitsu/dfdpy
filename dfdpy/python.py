...
def make_dfd(source: str, hidden_id_list: Sequence[str]):
    ...
    for name in tree._fields:
        for node in getattr(tree, name):
            if isinstance(node, ast.Assign):
                ...
                print(
                    f"style {node_abbreviation} text-align:left,fill:#ffffde")
                ...
                for lhs_node in lhs_node_list:
                    for rhs_node in rhs_node_list:
                        ...
                        lhs_node_id_with_version = str(
                            lhs_node.id) + "'"*shown_node_id_with_version[lhs_node.id]
                        ...
                        if (node_abbreviation, lhs_node_id_with_version) not in already_written_edge:
                            print(
                                f'{node_abbreviation} --> {lhs_node_id_with_version};')
                            already_written_edge.append(
                                (node_abbreviation, lhs_node_id_with_version))
